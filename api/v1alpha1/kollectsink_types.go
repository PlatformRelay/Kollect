...
type TLSSpec struct {
	...
	// and therefore one transport, so this never applies to both at once. Off by default. Requires
	// the operator to start the manager with --allow-insecure-sinks: admission rejects the field and
	// sink construction refuses it otherwise. When set, the TLSInsecure status condition is surfaced
	// on the sink independently of the connection probe, so its presence is reliable evidence the
	// field is set. See ADR-0104 and ADR-0407.
	// +optional
