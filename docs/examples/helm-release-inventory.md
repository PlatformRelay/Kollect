...
`config/samples/advanced/kollect_v1alpha1_kollectinventory.yaml` — its `databaseSinkRefs` entry names
the sink as an object with a `name` key. The CRD rejects the bare-string form.
