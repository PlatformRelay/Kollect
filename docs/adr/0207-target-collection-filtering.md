...
```mermaid
flowchart TD
  Objects["Informer events"] --> Cache["Operator watch scope"]
  Cache --> Deny["Scope namespace deny / allow"]
  Deny --> Intent["Target include / exclude"]
  Intent --> Rules["Resource rules · CEL match policy"]
  Rules --> GVK["Scope allowed GVKs"]
  GVK --> Labels["Watch labels"]
  Labels --> Rows["Inventory rows"]
```
