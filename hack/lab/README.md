...
live Kind capture uses localhost port-forward + `curl`/`go tool pprof` (maintainer opt-in).

```sh
task perf-kind:quick
bash hack/lab/perf-kind.sh --dry-run --run-id perf-demo --objects 500 --seed 42 --duration 60s
bash hack/lab/perf-kind.sh --run-id perf-live --objects 500   # requires kind-* context + pprof.enabled
```
